...
@Component({
  selector: 'app-product-borrow-carousel',
    imports: [
        NgForOf,
        NgIf
    ],
  templateUrl: './product-borrow-carousel.html',
  styleUrl: './product-borrow-carousel.css',
})
export class ProductBorrowCarousel implements OnInit {
  ...
  loadBooks() {
    this.loading = true;
    this.productService.getTopBorrowProduct().subscribe({
      next: (res) => {
        this.books = res.data || [];
  ...
  triggerAnimation() {
    const el = document.querySelector('#borrowBooksCarousel') as HTMLElement;
    if (!el) return;
